...
class TestRegistryDetection:
    ...
    def test_unsafe_superseded_flagged(self):
        ...
        assert any(x["pattern_id"] == "ADP3-DG-SUPERSEDED" for x in f), f"Superseded not flagged"

    def test_unsafe_receipt_authority_flagged(self):
        f = _findings(str(RFX / "unsafe-registry.jsonl"))
        assert any(x["pattern_id"] == "ADP3-DG-RECEIPT-AUTH" for x in f), f"Receipt authority not flagged"

    def test_unsafe_ai_read_archived_flagged(self):
        f = _findings(str(RFX / "unsafe-registry.jsonl"))
        assert any(x["pattern_id"] == "ADP3-DG-AI-STALE" for x in f), f"AI read archived not flagged"
