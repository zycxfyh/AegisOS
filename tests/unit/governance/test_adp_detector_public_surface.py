...
class TestPublicSurfaceDetection:
    ...
    def test_unsafe_release_flagged(self):
        ...
        assert any(x["pattern_id"] == "ADP3-PV-RELEASE" for x in f), f"Release overclaim not flagged"

    def test_unsafe_ready_flagged(self):
        f = _findings(str(PFX / "unsafe-pv-doc.md"))
        assert any(x["pattern_id"] == "ADP3-PV-READY" for x in f), f"READY approval not flagged"

    def test_unsafe_wedge_collapse_flagged(self):
        f = _findings(str(PFX / "unsafe-pv-doc.md"))
        assert any(x["pattern_id"] == "ADP3-PV-WEDGE" for x in f), f"Wedge collapse not flagged"
