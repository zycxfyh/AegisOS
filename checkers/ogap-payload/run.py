...
def run():
    ...
    for fpath in sorted(FIXTURES_DIR.rglob("*.json")):
        ...
        errors = []
        if "decision" in payload and payload.get("decision", "") not in {
            "READY",
            "DEGRADED",
            "BLOCKED",
            "HOLD",
...
if __name__ == "__main__":
    r = run()
    print(
        f"OGAP fixtures: {r.stats.get('files', 0)} files, {r.stats.get('valid', 0)} valid, {r.stats.get('invalid', 0)} invalid"
    )
    for f in r.findings:
        print(f"  {f}")
