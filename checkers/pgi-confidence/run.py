"""PGI Confidence Validator — validates PGI fixture payloads."""
...
FIXTURES = ROOT / "tests" / "fixtures" / "pgi_confidence_assessment"
...
if __name__ == "__main__":
    r = run()
    label = "PGI"
    print(f"{label}: {r.stats.get('files', 0)} files | Invalid: {r.stats.get('invalid', 0)}")
    for f in r.findings:
        print(f"  {f}")
