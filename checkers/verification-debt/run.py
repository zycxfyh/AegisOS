...
def run():
    ...
    for e in entries:
        ...
        if did in ids:
            errors.append(f"{did}: dup")
            ids.add(did)
        if (c := e.get("category", "")) not in VALID_CAT:
            errors.append(f"{did}: bad cat {c}")
        if (s := e.get("severity", "")) not in VALID_SEV:
            errors.append(f"{did}: bad sev {s}")
        st = e.get("status", "")
        if st not in VALID_STS:
            errors.append(f"{did}: bad status {st}")
        if st in ("open", "accepted_until"):
            stats["open"] += 1
        if st == "closed":
            stats["closed"] += 1
