...
def run():
    ...
    for e in entries:
        did = e.get("doc_id", "?")
        if m := REQUIRED_FIELDS - set(e.keys()):
            errors.append(f"{did}: missing {m}")
        if did in ids:
            errors.append(f"{did}: duplicate")
        ids.add(did)
        if e.get("doc_type", "") not in VALID_DOC_TYPES:
            errors.append(f"{did}: bad doc_type")
        if e.get("authority", "") not in VALID_AUTHORITIES:
            errors.append(f"{did}: bad authority")
        if e.get("status") == "stale":
            stats["stale_entries"] += 1
        if (p := e.get("path", "")) and not (ROOT / p).exists():
            stats["missing_files"] += 1
            errors.append(f"{did}: missing file {p}")
        for pr, desc in DANGEROUS:
            for fld in ("title", "notes"):
                if pr.search(str(e.get(fld, ""))):
                    errors.append(f"{did}: {desc}")
