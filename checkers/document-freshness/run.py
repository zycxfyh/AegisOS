...
def run():
    ...
    for e in entries:
        ...
        sa = e.get("stale_after_days")
        if not lv:
            if e.get("authority", "") in CRITICAL:
                stats["no_last_verified"] += 1
                findings.append(f"{did}: missing last_verified (critical)")
            continue
        stats["with_freshness"] += 1
        try:
            lvd = date.fromisoformat(lv)
        except:
            findings.append(f"{did}: bad date {lv}")
            continue
        w = sa if isinstance(sa, int) else 14
        age = (today - lvd).days
        if age > w * 2:
            stats["stale"] += 1
            if e.get("authority", "") in CRITICAL:
                stats["stale_critical"] += 1
                findings.append(f"{did}: CRITICAL STALE {age}d (window={w}d)")
            else:
                findings.append(f"{did}: stale {age}d (window={w}d)")
        elif age > w:
            findings.append(f"{did}: approaching stale {age}d (window={w}d)")
...
if __name__ == "__main__":
    ...
    if len(r.findings) > 20:
        print(f"    ... +{len(r.findings) - 20} more")
