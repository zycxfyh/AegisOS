...
def run():
    ...
    try:
        ...
        return CheckerResult("fail", 1, [f"Invalid JSON: {e}"])
    gates = m.get("gates", [])
    stats["gates"] = len(gates)
    if m.get("gate_count", 0) != len(gates):
        errors.append("gate_count mismatch")
    ids = set()
    for g in gates:
        gid = g.get("gate_id", "?")
        if gid in ids:
            errors.append(f"dup: {gid}")
            ids.add(gid)
        ...
        for fld in ("gate_id", "display_name", "layer", "hardness", "command"):
            if fld not in g:
                errors.append(f"{gid}: missing {fld}")
    return CheckerResult("fail" if errors else "pass", 1 if errors else 0, errors, stats)


if __name__ == "__main__":
    r = run()
    print(f"  Gates: {r.stats.get('gates', 0)} | Violations: {len(r.findings)}")
    for f in r.findings:
        print(f"    {f}")
