...
## Governed Layer (917 registry objects)
...
| `artifact-registry.jsonl` | 651 entries | Code files (tests, scripts, domains, src) with `artifact_class` × `criticality` × `layer` |
...
tests/            412 files  →  artifact-registry.jsonl  (test files)
