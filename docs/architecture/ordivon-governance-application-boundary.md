...
## Governed Layer (923 registry objects)
...
| `artifact-registry.jsonl` | 657 entries | Code files (tests, scripts, domains, src) with `artifact_class` × `criticality` × `layer` |
...
tests/            418 files  →  artifact-registry.jsonl  (test files)
...
checkers/          38 pkgs   →  checker_registry.py      (verification checkers)
```
