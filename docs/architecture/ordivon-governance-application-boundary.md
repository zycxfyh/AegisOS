...
## Governed Layer (929 registry objects)
...
| `artifact-registry.jsonl` | 663 entries | Code files (tests, scripts, domains, src) with `artifact_class` × `criticality` × `layer` |
...
tests/            424 files  →  artifact-registry.jsonl  (test files)
