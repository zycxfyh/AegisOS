...
## 1. Checker Ecosystem (40 checkers, L3-L10)
...
L6     Document Registry    [HARD]  Document registry invariants (derived from document-registry.jsonl — current count varies)
...
in `docs/governance/document-registry.jsonl` (derived count; see registry for current entry count) with structured
...
uv run python scripts/run_baseline.py               # Full baseline (40 checkers; writes telemetry/shadow ledgers)
