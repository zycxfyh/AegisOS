...
def valid_payload(**overrides):
    p = {
        ...
        "stop_loss": "Below support", "target": "Local high",
        "position_size_usdt": 100.0, "max_loss_usdt": 20.0, "risk_unit_usdt": 10.0,
        "is_revenge_trade": False, "is_chasing": False,
        "emotional_state": "calm", "confidence": 0.7,
        "rule_exceptions": [], "notes": "Controlled",
    }
    p.update(overrides)
    return p
...
if gov1.get("governance_decision") == "execute":
    ...
    if "execution_receipt_id" in p1:
        ...
        check("H-9C1.3: outcome created",
              "outcome_id" in o1,
              f"outcome_id={o1.get('outcome_id', 'MISSING')}")
        ...
        if "outcome_id" in o1:
            ...
            check("H-9C1.4: review submitted (outcome_ref in DB, API echo known gap)",
                  rv1.get("id") is not None,
                  f"review_id={rv1.get('id')}")

            if rv1.get("id"):
                cp1 = complete_review(rv1["id"], {
                ...
                check("H-9C1.5: review completed",
...
check("H-9C2.1: emotional_state 'stressed' → escalate",
...
check("H-9C2.2: emotional_state 'fearful' → escalate",
...
check("H-9C2.3: emotional_state 'calm' → execute (not escalate)",
...
check("H-9C2.4: rule_exceptions not empty → escalate",
...
check("H-9C2.5: confidence=0.2 → escalate",
...
check("H-9C2.6: confidence=0.5 → execute (not escalated for confidence)",
...
check("H-9C2.7: priority: reject beats escalate (missing stop_loss + stressed)",
...
check("H-9C3.1: banned thesis 'just feels right' → reject",
...
check("H-9C3.2: banned thesis 'vibes...trust me' → reject",
...
check("H-9C3.3: short thesis (no banned patterns) → escalate",
...
check("H-9C3.4: no verifiability wording → escalate",
...
check("H-9C3.5: valid thesis (confirmation + invalidation) → execute",
...
check("H-9C3.6: YOLO thesis → reject",
