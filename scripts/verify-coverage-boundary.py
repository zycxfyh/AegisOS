    ...
    for f in data.get("files", []):
        ...
        if status == "blocked":
            ...
            if not meta.get("reason") or len(str(meta.get("reason", ""))) < 3:
                findings.append({"code": "CB-2", "severity": "degraded", "path": path, "message": "Exclusion missing reason"})
        elif status == "debt_parked":
            if not meta.get("debt_id"):
                findings.append({"code": "CB-4", "severity": "blocking", "path": path, "message": "Debt-parked without debt_id"})
        elif status == "debt_or_exclusion_required":
            findings.append({"code": "CB-12", "severity": "degraded", "path": path, "message": "Non-protected file requires debt or exclusion"})
