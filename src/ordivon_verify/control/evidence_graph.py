...
class EvidenceGraph:
    ...
    def add_evidence(self, evidence_id: str, evidence_type: str,
                     path: str, **meta) -> EvidenceNode:
        node = EvidenceNode(evidence_id, "evidence",
                            f"{evidence_type}: {path}",
                            {"type": evidence_type, "path": path, **meta})
        self.nodes.append(node)
        return node

    def add_verification(self, gate_id: str, passed: bool, **meta) -> EvidenceNode:
        node = EvidenceNode(gate_id, "verification",
                            f"{'✅' if passed else '❌'} {gate_id}",
                            {"passed": passed, **meta})
        self.nodes.append(node)
        return node
    ...
    def from_receipt(cls, receipt: dict) -> EvidenceGraph:
        ...
        for v in receipt.get("verification_results", []):
            ...
            g.add_verification(v_id, v.get("passed", False),
                               exit_code=v.get("exit_code"),
                               hash=v.get("output_hash", ""))
        ...
        for f in receipt.get("files_changed", []):
            ...
            g.link(f"claim:{stage_id}:all_required_docs_registered",
                   art_id, "references")
