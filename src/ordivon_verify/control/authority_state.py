...
VALID_AUTH_TRANSITIONS = {
    ...
    AuthorizationStatus.REQUESTED:     {AuthorizationStatus.REQUESTED, AuthorizationStatus.APPROVED, AuthorizationStatus.REJECTED, AuthorizationStatus.EXPIRED},
...
class AuthorityState:
    ...
    def transition_evidence(self, target: EvidenceStatus, reason: str = "") -> AuthorityState:
        if target not in VALID_EVIDENCE_TRANSITIONS.get(self.evidence, set()):
            raise InvalidTransitionError(
                f"Evidence: {self.evidence.value} → {target.value} is not allowed"
            )
        return AuthorityState(evidence=target, readiness=self.readiness,
                              authorization=self.authorization, policy=self.policy)

    def transition_authorization(self, target: AuthorizationStatus, approver: str = "",
                                 rationale: str = "") -> AuthorityState:
        if target not in VALID_AUTH_TRANSITIONS.get(self.authorization, set()):
            raise InvalidTransitionError(
                f"Authorization: {self.authorization.value} → {target.value} is not allowed"
            )
        return AuthorityState(evidence=self.evidence, readiness=self.readiness,
                              authorization=target, policy=self.policy)
    ...
    def transition_policy(self, target: PolicyStatus) -> AuthorityState:
        if target not in VALID_POLICY_TRANSITIONS.get(self.policy, set()):
            raise InvalidTransitionError(
                f"Policy: {self.policy.value} → {target.value} is not allowed"
            )
        return AuthorityState(evidence=self.evidence, readiness=self.readiness,
                              authorization=self.authorization, policy=target)
